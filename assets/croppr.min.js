...
class SynchroCropprInputs {
  ...
  initializeCroppr() {
    ...
    if (!imageElement) {
      console.warn('Croppr image element not found');
      return;
    }

    // Fonction pour initialiser Croppr une fois l'image chargée
    const initCroppr = () => {
      ...
      if (imageElement.naturalWidth > 0 && imageElement.naturalHeight > 0) {
        ...
        // Surveiller les changements d'attributs de l'image
        this.observeImageChanges(imageElement);
      } else {
        // Si l'image n'a pas encore de dimensions, attendre un peu plus
        setTimeout(initCroppr, 100);
      }
    };
    ...
    if (imageElement.complete && imageElement.naturalWidth > 0) {
      ...
      imageElement.addEventListener('load', initCroppr);
      // Fallback au cas où l'événement load ne se déclenche pas
      setTimeout(initCroppr, 2000);
    }
  }
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && 
          ...
          // L'image a changé, réinitialiser Croppr
          if (this.croppr) {
            this.croppr.destroy();
          }
  ...
  handleInputChange(detail) {
    ...
    // Réinitialiser avec le nouveau ratio
    const imageElement = document.getElementById('croppr');
    if (imageElement) {
      // Attendre que l'image soit prête avant de réinitialiser
      const reinitCroppr = () => {
        if (imageElement.naturalWidth > 0 && imageElement.naturalHeight > 0) {
          this.croppr = new Croppr('#croppr', {
  ...
  updateInputsPosition = (instance) => {
    ...
    this.inputXPosition = this.inputXPosition ?? document.querySelector('input[name="tapestry-x-position"]');
    this.inputYPosition = this.inputYPosition ?? document.querySelector('input[name="tapestry-y-position"]');
  ...
  // Fonction pour forcer la réinitialisation en cas de problème
  forceReinitialize() {
...
// Gestionnaire pour forcer la réinitialisation en cas de problème
document.addEventListener('DOMContentLoaded', () => {
  // Vérifier périodiquement si Croppr fonctionne correctement
  setInterval(() => {
    const cropprContainer = document.querySelector('.croppr-container');
...
class TapestrySelect extends HTMLElement {
  ...
  setInputRatioValue() {
    this.inputRatio = this.inputRatio ?? document.querySelector('input[name="tapestry-ratio"]');

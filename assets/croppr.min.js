...
class SynchroCropprInputs {
  ...
  handleInputChange(detail) {
    ...
    this.croppr = new Croppr('#croppr', {
      ...
      maxSize: ['100%', '100%'],
      onInitialize: (instance) => this.updateInputsPosition(instance),
      onCropStart: () => this.fireCropRegionChangeEvent(),
      onCropEnd: () => this.updateInputsPosition()
    });
  }
...
new SynchroCropprInputs();

class TapestrySelect extends HTMLElement {
  constructor() {
    super();
    this.select = this.querySelector('select');
    this.selects = Array.from(document.querySelectorAll('tapestry-select select'));
  ...
  setInputCm2Value() {
    this.inputCm2 = this.inputCm2 ?? document.querySelector('input[name="tapestry-cm2"]');
    const {width, height} = this.getSize(); 
    const cm2 = Math.round(width * height);   
    this.inputCm2.value = cm2
  }
  ...
  async setProductPrice() {
    window.tapestryPriceM2 = window.tapestryPriceM2 ?? await this.getProductPrice();
    this.priceCm2 = this.priceCm2 ?? window.tapestryPriceM2 / 10000;

    const { width, height } = this.getSize();   
    const price = width * height * this.priceCm2;
    const priceFormated = price % 1 !== 0 ? price.toFixed(1) : price
    const finalPrice = isNaN(priceFormated) ? '' : `${this.moneySymbol}${priceFormated}`;
    this.priceElem = this.priceElem ?? document.getElementById('main-price');
    this.priceElem.textContent = finalPrice;
  }

  async getProductPrice() {
    const serverUrl = 
      Shopify.theme.role === 'development' ? 
        'http://localhost:3333' : 
        'https://backend.myselfmonart.com';
    
    const response = await fetch(serverUrl + '/api/tapestry/price', {
      method: 'GET',
  ...
  setNumberOfTapestryStrips() {
    this.stripsElem = this.stripsElem ?? document.getElementById('nb-tapestry-strips');
    this.stripsElem.closest('li').classList.remove('hidden');
    ...
    this.stripsElem.textContent = this.numberOfStrips;
  }
  ...
  getSize() {
    this.selectWidth = this.selectWidth ?? this.selects.find(select => select.id === 'tapestry-width');
    this.selectHeight = this.selectHeight ?? this.selects.find(select => select.id === 'tapestry-height');
    return {

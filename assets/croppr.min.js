...
class SynchroCropprInputs {
  constructor() {
    this.isInCarousel = !!document.querySelector('tapestry-product-carousel');
    this.isInitializing = false;
    this.isInitialized = false;
    this.fallbackTimeout = null;
    this.retryTimeout = null;
    this.initializeCroppr();

    if (this.isInCarousel) {
      this.setupCarouselAwareness();
    }
  }

  setupCarouselAwareness() {
    // Use IntersectionObserver to detect when cropper is visible in carousel
    const cropperContainer = document.getElementById('croppr-top-container');
    if (!cropperContainer) return;

    this.visibilityObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && entry.intersectionRatio > 0.5) {
          this.ensureCropperSized();
        }
      });
    }, { threshold: [0.5] });

    this.visibilityObserver.observe(cropperContainer);

    // Listen to carousel scroll events
    document.addEventListener('tapestryCarouselScroll', ({ detail }) => {
      if (detail.isCropperVisible) {
        this.ensureCropperSized();
      }
    });
  }
  ...
  ensureCropperSized() {
    // Force a layout recalculation when cropper becomes visible
    if (this.croppr) {
      const tapestrySelect = document.querySelector('tapestry-select');
      if (tapestrySelect) {
        tapestrySelect.setCropprLayout();
      }
    }
  }
  ...
  initializeCroppr() {
    ...
    const imageElement = document.getElementById('croppr');
    if (!imageElement) {
      // Element not found - this is expected on non-tapestry pages
      return;
    }

    this.isInitializing = true;

    // Function to initialize Croppr once the image is loaded
    const initCroppr = () => {
      ...
      if (imageElement.naturalWidth > 0 && imageElement.naturalHeight > 0) {
        // Clear any pending timeouts since we're initializing now
        this.clearPendingTimeouts();

        // Add loading class
        const container = imageElement.parentElement;
        if (container) {
          container.classList.add('croppr-loading');
        }

        try {
          this.croppr = new Croppr('#croppr', {
            aspectRatio: 2.5,
            onInitialize: () => this.fixCropRegionSizeOnMobile(),
            onCropStart: () => this.fireCropRegionChangeEvent(),
            onCropEnd: () => this.updateInputsPosition()
          });

          this.isInitialized = true;
          this.isInitializing = false;

          const initialValues = this.croppr.getValue();
          this.initialWidth = initialValues.width;
          this.initialHeight = initialValues.height;
          this.initialRatio = Number(parseFloat(this.initialWidth / this.initialHeight).toFixed(2));

          document.addEventListener('tapestrySizeChange', ({detail}) => this.handleInputChange(detail));

          window.addEventListener('resize', () => {
            document.querySelector('tapestry-select')?.setCropprLayout();
          });

          // Watch for image attribute changes
          this.observeImageChanges(imageElement);
        } catch (e) {
          console.warn('Croppr initialization failed:', e);
          this.isInitializing = false;
        }
      } else {
        // If the image doesn't have dimensions yet, wait a bit more
        this.retryTimeout = setTimeout(initCroppr, 100);
      }
    };
  ...
  observeImageChanges(imageElement) {
    // Watch for image attribute changes
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' &&
            (mutation.attributeName === 'src' || mutation.attributeName === 'srcset')) {
          // Image has changed, reinitialize Croppr
          if (this.croppr) {
            this.croppr.destroy();
            this.croppr = null;
          }
          ...
          this.isInitialized = false;
          this.isInitializing = false;
          setTimeout(() => this.initializeCroppr(), 100);
        }
      });
    });
  ...
  handleInputChange(detail) {
    ...
    // Reinitialize with the new ratio
    const imageElement = document.getElementById('croppr');
    if (!imageElement) return;

    // Wait for the image to be ready before reinitializing
    const reinitCroppr = () => {
      if (imageElement.naturalWidth > 0 && imageElement.naturalHeight > 0) {
        try {
          this.croppr = new Croppr('#croppr', {
            aspectRatio: detail.ratio,
            maxSize: ['100%', '100%'],
            onInitialize: (instance) => {
              this.updateInputsPosition(instance);
              this.cleanupOverlay();
            },
            onCropStart: () => this.fireCropRegionChangeEvent(),
            onCropEnd: () => this.updateInputsPosition()
          ...
          console.warn('Croppr reinitialization failed:', e);
        }
      } else {
        setTimeout(reinitCroppr, 50);
      }
    };
  ...
  fixCropRegionSizeOnMobile() {
    const imageClipped = document.querySelector('.croppr-imageClipped');
    imageClipped.addEventListener('load', () => {
      const clipValue = imageClipped.style.clip;
      if(clipValue === 'rect(0px, 0px, 0px, 0px)') {
        document.querySelector('tapestry-select').setCropprLayout();   
      }
    });
    
    // Clean up the overlay if it appears
    this.cleanupOverlay();
  }

  cleanupOverlay() {
    // Wait a bit for Croppr to be fully initialized
    setTimeout(() => {
      const cropprContainer = document.querySelector('.croppr-container');
      if (cropprContainer) {
        // Add class to indicate Croppr is ready
        cropprContainer.classList.add('croppr-ready');
        cropprContainer.classList.remove('croppr-loading');
        
        // Ensure the overlay is properly configured
        const overlay = cropprContainer.querySelector('.croppr-overlay');
        if (overlay) {
          overlay.style.pointerEvents = 'auto';
        }
        
        // Check that the original image doesn't have incorrect opacity
        const originalImage = cropprContainer.querySelector('.croppr img:first-child');
        if (originalImage) {
          originalImage.style.opacity = '0.2';
        }
        
        // Ensure the clipped image is visible
        const clippedImage = cropprContainer.querySelector('.croppr-imageClipped');
        if (clippedImage) {
          clippedImage.style.opacity = '1';
        }
      }
    }, 100);
  }

  updateInputsPosition = (instance) => {
    const croppr = instance ?? this.croppr;
    this.inputXPosition = this.inputXPosition ?? document.querySelector('input[name="_tapestry-x-position"]');
    this.inputYPosition = this.inputYPosition ?? document.querySelector('input[name="_tapestry-y-position"]');
    const { x, y } = croppr.getValue('ratio');
    this.inputXPosition.value = isNaN(x) ? 0 : x;
    this.inputYPosition.value = isNaN(y) ? 0 : y;
  }

  fireCropRegionChangeEvent() {
    const event = new CustomEvent('cropRegionChange', {});
    document.dispatchEvent(event);
  }

  // Function to force reinitialization in case of problems
  forceReinitialize() {
    // Don't force reinitialize if already initializing
    if (this.isInitializing) return;

    console.log('Forcing Croppr reinitialization...');
    if (this.croppr) {
      this.croppr.destroy();
      this.croppr = null;
    }
    ...
    // Clean up classes
    const container = document.querySelector('.croppr-container');
    if (container) {
      container.classList.remove('croppr-ready', 'croppr-loading');
    }
...
const synchroCroppr = new SynchroCropprInputs();

// Handler to force reinitialization in case of problems
document.addEventListener('DOMContentLoaded', () => {
  // Periodically check if Croppr is working correctly
  setInterval(() => {
    // Skip check if initialization is in progress or not yet attempted
    if (synchroCroppr.isInitializing) return;

    const cropprContainer = document.querySelector('.croppr-container');
    const cropprRegion = document.querySelector('.croppr-region');

    // Only force reinitialize if container exists but region doesn't (corrupted state)
    if (cropprContainer && !cropprRegion && !cropprContainer.classList.contains('croppr-loading')) {
      console.warn('Croppr region not found, forcing reinitialization');
      synchroCroppr.forceReinitialize();
    }
  }, 5000); // Check every 5 seconds
});

class TapestrySelect extends HTMLElement {
  constructor() {
    super();
    this.select = this.querySelector('select');
    this.selectWidth = document.getElementById('tapestry-width')
    this.selectHeight = document.getElementById('tapestry-height')
    this.moneySymbol = this.dataset.moneySymbol;
    this.orientation = this.dataset.orientation
    this.stripsWidth = this.dataset.tapestryStripsWidth;
  ...
  async connectedCallback() {
    ...
    if(this.orientation === 'width') {
      const mutationObserver = new MutationObserver(this.listenCropRegionExistence.bind(this));
      mutationObserver.observe(document.getElementById('croppr-top-container'), {
        childList: true,
        subtree: true
      });
    
      const cropRegion = document.querySelector('.croppr-region')
      this.currentElement = cropRegion;
      this.attachResizeObserver(cropRegion);
    }
  ...
  setAllData() {
    this.setInputRatioValue();
    this.setCropprLayout();
    this.setProductPrice();
    this.setVariantSelectedInBuyButton();
    this.setNumberOfTapestryStrips();
  }

  listenCropRegionExistence(mutations) {
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === Node.ELEMENT_NODE && node.classList.contains('croppr-container')) {
          const cropprRegionElem = node.querySelector('.croppr-region');
          this.attachResizeObserver(cropprRegionElem);
        }
      });
  ...
  attachResizeObserver(element) {
    ...
    this.resizeObserver = new ResizeObserver(() => {
      this.setTapestryStripsImitation();  
      this.setTapestryTexture();
      document.dispatchEvent(new CustomEvent('tapestrySizeChanged'));  
    });
  ...
  setInputRatioValue() {
    this.inputRatio = this.inputRatio ?? document.querySelector('input[name="_tapestry-ratio"]');
    this.inputRatio.value = this.getRatio();
  }

  setCropprLayout() {
    const ratio = this.getRatio();
    document.dispatchEvent(new CustomEvent('tapestrySizeChange', { 
      detail: { 
        ratio: Number(parseFloat(1 / ratio)) 
      } 
    }));
  }
  ...
  setProductPrice() { 
    ...
    this.priceElem = this.priceElem ?? document.querySelector('.main-price');
    this.priceElem.textContent = this.price;
  }

  getProductPrice() {
    const variant = this.getVariantSelected()
    return Product.formatPrice(variant.price)
  }

  getVariantSelected() {
    return window.variants.find(variant => variant.option1 === this.selectWidth.value && variant.option2 === this.selectHeight.value)
  }

  setNumberOfTapestryStrips() {
    const { width } = this.getSize();
    this.numberOfStripsExact = Number((width / this.stripsWidth).toFixed(2));
    this.numberOfStrips = Math.ceil(this.numberOfStripsExact);
  }

  setTapestryStripsImitation() {
    const cropprRegionElem = this.cropprRegionElem ?? document.querySelector('.croppr-region');
    const relativeDiv = document.createElement('div');
    relativeDiv.classList.add('tapestry-strips-container', 'block', 'relative', 'h-full', 'overflow-hidden');
    cropprRegionElem.appendChild(relativeDiv);

    const cropprRegionBonding = document.querySelector('.croppr-region').getBoundingClientRect();
    const cropprRegionWidth = cropprRegionBonding.width;
    const stripsWidthInPx = Math.round(cropprRegionWidth / this.numberOfStripsExact);
    
    for (let i = 1; i < this.numberOfStrips; i++) {
      const tapestryStrip = document.createElement('div');
      tapestryStrip.classList.add('!block', 'absolute', 'bg-white/10', 'h-full', 'top-0');
      tapestryStrip.style.width = '1px';
      tapestryStrip.style.left = i * stripsWidthInPx + 'px';
      relativeDiv.appendChild(tapestryStrip);
    }
  }

  setTapestryTexture() {
    const container = document.querySelector('.tapestry-strips-container');
    const template = document.getElementById('tapestry-texture');
    if(!template) return;
  ...
  getSize() {
    return {
      width: Number(parseFloat(this.selectWidth.value).toFixed(1)),
      height: Number(parseFloat(this.selectHeight.value).toFixed(1)),
    };
  }

  setVariantSelectedInBuyButton() {
    const buyButton = this.buyButton ?? document.querySelector('.buy-button')
    const variantSelected = this.getVariantSelected()
    buyButton.dataset.variantId = variantSelected.id

    let floatBuyButton = document.querySelector('.float-buy-button');
    if (!floatBuyButton) {
      const templateFloatBuyButton =
        document.getElementById('float-buy-button');
      const content = templateFloatBuyButton.content;
      floatBuyButton = content.querySelector('.float-buy-button');
    }
    floatBuyButton.dataset.variantId = variantSelected.id
  }
}
customElements.define('tapestry-select', TapestrySelect);

class FingerTouch extends HTMLElement {
  constructor() {
    super();
    this.paintingRatio = this.dataset.paintingRatio;
    this.isInCarousel = !!document.querySelector('tapestry-product-carousel');
  }

  connectedCallback() {
    document.addEventListener('cropRegionChange', this.hideAnimation);
    document.addEventListener('tapestrySizeChanged', this.showAnimation);

    // Handle carousel visibility - hide animation when scrolled away from cropper
    if (this.isInCarousel) {
      document.addEventListener('tapestryCarouselScroll', ({ detail }) => {
        if (!detail.isCropperVisible) {
          this.hideAnimation();
        } else {
          // Show animation when returning to cropper
          this.showAnimation();
        }
      });
    }
  ...
  hideAnimation = () => {
    this.logo = this.logo ?? document.querySelector('.finger-touch');
    if (this.logo) {
  ...
  showAnimation = () => {
    ...
    if(direction === 'horizontal') {
      this.logo.classList.replace('finger-y-touch', 'finger-x-touch');
    } else if(direction === 'vertical') {
      this.logo.classList.replace('finger-x-touch', 'finger-y-touch');
    } else {
      this.logo.classList.remove('finger-x-touch', 'finger-y-touch');
    }
    this.logo.classList.remove('hidden');
  }

  getDirection() {
    this.container = document.querySelector('.croppr');
    const cropRegion = document.querySelector('.croppr-region');

    if (!this.container || !cropRegion) return 'square';

    const containerWidth = this.container.offsetWidth;
    const containerHeight = this.container.offsetHeight;
    const cropRegionWidth = cropRegion.offsetWidth;
    const cropRegionHeight = cropRegion.offsetHeight;

    if(containerWidth > cropRegionWidth) return 'horizontal'
    else if(containerHeight > cropRegionHeight) return 'vertical'
    else return 'square'
  }
}
customElements.define('finger-touch', FingerTouch);